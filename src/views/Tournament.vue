    ...
    <v-row dense>
      <v-col>
        ...
        <v-dialog v-model="showPredictionDialog">
          <v-card class="pt-5 pb-5 px-5 text-center" style="position: relative">
          ...
          >
            ...
            >
              ...
              <div v-if="f.fixture.status.short === 'NS'">
                ...
                <p
                  class="text-caption mt-1 text-grey-darken-1 font-weight-medium"
                >
            ...
            <template v-if="authenticated">
              <div
                ...
                class="my-4 text-subtitle-2 d-flex justify-space-between align-center"
              >
                <p>Your Prediction</p>
                <p>
                  <span
...
import PlayerOfTheMatch from '@/components/PlayerOfTheMatch.vue';
...
export default {
  ...
  data: () => ({
    ...
    potm: { name: null, team: null, id: null, number: null },
    gameWeekForSelect: [],
    playerOptions: [],
    fixtureTeamOptions: [],
  }),
  methods: {
    ...
    setPOTM(player) {
      this.predictionForm.playerOfTheMatch = player.id;
      this.potm.name = player.name;
      this.potm.number = player.number;
      this.potm.id = player.id;
      this.potm.team = player.team;
    },
    async predictionDialogHandler(f) {
      ...
      if (!this.authenticated) {
        ...
        if (getFixtureDetailResponse.results[0].lineups.length > 0) {
          const homeTeamPlayers = [
            ...getFixtureDetailResponse.results[0].lineups[0].startXI.map(
              (p) => {
                return p.player;
              }
            ),
            ...getFixtureDetailResponse.results[0].lineups[0].substitutes.map(
              (p) => {
                return p.player;
              }
            ),
          ...
          const awayTeamPlayers = [
            ...getFixtureDetailResponse.results[0].lineups[1].startXI.map(
              (p) => {
                return p.player;
              }
            ),
            ...getFixtureDetailResponse.results[0].lineups[1].substitutes.map(
              (p) => {
                return p.player;
              }
            ),
          ...
          this.playerOptions = [
            { team: f.teams.home.name, players: homeTeamPlayers },
            { team: f.teams.away.name, players: awayTeamPlayers },
          ];
          console.log(
            '🚀 ~ file: Tournament.vue:558 ~ predictionDialogHandler ~ playerOptions',
            this.playerOptions
          );
        }
        ...
        if (this.getFixturePrediction(f.fixture.id)) {
          ...
          this.predictionForm.winnerTeam = parseInt(
            this.getFixturePrediction(f.fixture.id).winner
          );
          this.predictionForm.playerOfTheMatch = parseInt(
            this.getFixturePrediction(f.fixture.id).playerOfTheMatch
          );
        } else {
          this.predictionForm.teamOnePredictionNumber = ['0'];
          this.predictionForm.teamTwoPredictionNumber = ['0'];
    ...
    updatePredictionValue(fixture_id, data) {
      ...
      if (predictions.length <= 0) {
        ...
        if (checkPredictionExist) {
          ...
          predictions[existingPredictionIndex].playerOfTheMatch = parseInt(
    ...
    async onSavePredictionHandler() {
      ...
      const response = await this.savePredictionAction({
        ...
        winner:
          this.predictionForm.teamOnePredictionNumber[0] ===
          this.predictionForm.teamTwoPredictionNumber[0]
            ? this.predictionForm.winnerTeam
            : null,
